...
export class ApiExampleService extends BaseApiService {
  ...
  findAll(name?: string): Observable<ExampleDto[]> {
    ...
    return this.http.get<ExampleDto[]>(`${this.BASE_URL}/examples/examples`, { params });
  }
  ...
  getCount(): Observable<{ count: number }> {
    return this.http.get<{ count: number }>(`${this.BASE_URL}/examples/examples/count`);
  }
  ...
  create(createExampleDto: Omit<ExampleDto, 'id'>): Observable<ExampleDto> {
    return this.http.post<ExampleDto>(`${this.BASE_URL}/examples/examples`, createExampleDto);
  }
  ...
  update(id: string, updateExampleDto: Partial<ExampleDto>): Observable<ExampleDto> {
    let url = `${this.BASE_URL}/examples/examples/:id`;
    url = url.replace(':id', id.toString());
    return this.http.put<ExampleDto>(url, updateExampleDto);
  }
  ...
  advancedUpdate(
    id: string,
    statusId: string,
    updateData: { notes?: string; metadata?: Record<string, any> },
    priority?: string,
    category?: string,
    assignee?: string,
    dueDate?: string,
    ...
    data: {
      ...
      assignee: string;
      dueDate: string;
      sessionInfo: { sessionId: string; userId?: string; timestamp: string };
      updateData: any;
      processedBy: string;
    };
  }> {
