...
export class ApiTodoItemService extends BaseApiService {
   ...
   */
  findAll(limit?: number, offset?: number, status?: "pending" | "in_progress" | "completed" | "archived", assignedTo?: string, tags?: string[]): Observable<TodoItemDto[]> {
    ...
    return this.http.get<TodoItemDto[]>(
      `${this.BASE_URL}/examples/todo-items`,
      { params }
    );
  }
  ...
  getCount(status?: "pending" | "in_progress" | "completed" | "archived"): Observable<{ count: number; }> {
    ...
    return this.http.get<{ count: number; }>(
      `${this.BASE_URL}/examples/todo-items/count`,
      { params }
    );
  }
  ...
  getOverdueTodos(): Observable<TodoItemDto[]> {
    return this.http.get<TodoItemDto[]>(
      `${this.BASE_URL}/examples/todo-items/overdue`,
    );
  }
  ...
  create(createTodoItemDto: CreateTodoItemDto): Observable<TodoItemDto> {
    return this.http.post<TodoItemDto>(
      `${this.BASE_URL}/examples/todo-items`,
      createTodoItemDto
    );
  }
  ...
  update(id: string, updateTodoItemDto: UpdateTodoItemDto): Observable<TodoItemDto> {
    let url = `${this.BASE_URL}/examples/todo-items/:id`;
    url = url.replace(':id', id.toString());
    return this.http.put<TodoItemDto>(
      url,
      updateTodoItemDto
    );
  }
  ...
  removeSubItem(id: string, subItemIndex: number): Observable<TodoItemDto> {
    ...
    return this.http.delete<TodoItemDto>(
      url,
    );
  }
