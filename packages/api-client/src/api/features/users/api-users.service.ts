...
import { ResetPasswordDto, ClientUserDto, UsersByChannelDto, UserInSessionDto } from '@medorion/types';
...
export class ApiUsersService extends BaseApiService {
  ...
  resetUserPassword(resetPasswordDto: ResetPasswordDto): Observable<void> {
    return this.http.post<void>(`${this.BASE_URL}/users/reset-password`, resetPasswordDto);
  }
  ...
  getUsers(): Observable<ClientUserDto[]> {
    return this.http.get<ClientUserDto[]>(`${this.BASE_URL}/users`);
  }
  ...
  getUsersByChannel(): Observable<UsersByChannelDto> {
    return this.http.get<UsersByChannelDto>(`${this.BASE_URL}/users/channel`);
  }
  ...
  sessions(lastSeconds: number): Observable<UserInSessionDto[]> {
    ...
    return this.http.get<UserInSessionDto[]>(url);
  }
}
