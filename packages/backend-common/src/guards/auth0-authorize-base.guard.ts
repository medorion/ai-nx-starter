...
export abstract class Auth0AuthorizeGuardBase implements CanActivate {
  ...
  constructor(
    ...
    this.auth0 = new ManagementClient({
      domain: this.configService.get<string>(EnvVariables.AUTH0_MANAGEMENT_DOMAIN) as string,
      clientId: this.configService.get<string>(EnvVariables.AUTH0_MANAGEMENT_CLIENT_ID) as string,
      clientSecret: this.configService.get<string>(EnvVariables.AUTH0_MANAGEMENT_CLIENT_SECRET) as string,
    });
  }
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (ignoreAuthorization) {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        const sessionInfo: SessionInfo = await this.getSessionInfo(decoded, request.headers['md-fp'], request.headers['x-forwarded-for']);
    ...
    const serverAuthorization = this.reflector.get<string>('serverAuthorization', context.getHandler());
    if (serverAuthorization) {
      request.session = await this.serverToServerAuthService.validateToken(request);
      return request.session != null;
    }

    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      this.logger.error(`Can Activate, Missing or invalid Authorization header`);
    ...
    const token = authHeader?.split(' ')[1];
    if (!token) {
      this.logger.error(`Can Activate, Token Missing for ${request.originalUrl}`);
      throw new ForbiddenException(`Token Missing for ${request.originalUrl}`);
    }

    const decoded = await this.getDecodedToken(token);
    const sessionInfo: SessionInfo = await this.getSessionInfo(decoded, fingerprint, ip);
    // Attach decoded token info to request/session
    request.session = sessionInfo; // Store mapped session info
    ...
    if (!fingerprint) {
      this.logger.error(`Can Activate, Missing fingerprint for ${request.originalUrl}`);
      throw new ForbiddenException(`Missing fingerprint for ${request.originalUrl}`);
    }
    if (sessionInfo.fingerprint !== fingerprint) {
      this.logger.error(`Can Activate, Fingerprint mismatch for ${request.originalUrl}`);
      throw new ForbiddenException(`Fingerprint mismatch for ${request.originalUrl}`);
    }

    const orgCode = request.params ? request.params[`${ORG_CODE_PATH_PARAM}`] : null;
    if (orgCode && !sessionInfo.availableOrganizations?.includes(orgCode) && sessionInfo.role !== Role.Root) {
      this.logger.error(`Can Activate, User ${sessionInfo.email} is not authorized for Organization ${orgCode}!`);
      throw new ForbiddenException(`Can Activate, User ${sessionInfo.email} is not authorized for Organization ${orgCode}!`);
    }
