...
export class Auth0AuthorizeGuard implements CanActivate {
  ...
  constructor(
    ...
    this.auth0 = new ManagementClient({
      domain: this.configService.get<string>('AUTH0_MANAGEMENT_DOMAIN') as string,
      clientId: this.configService.get<string>('AUTH0_MANAGEMENT_CLIENT_ID') as string,
      clientSecret: this.configService.get<string>('AUTH0_MANAGEMENT_CLIENT_SECRET') as string,
    });
  }
