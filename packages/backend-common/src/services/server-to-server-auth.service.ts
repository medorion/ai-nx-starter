...
export class ServerToServerAuthService {
  ...
  constructor(
    ...
    this.serverTokenExpiration =
      this.configService.get<number>(EnvVariables.TIME_BEFORE_SESSION_EXPIRE_MS) || ServerToServerAuthService.TIME_BEFORE_SESSION_EXPIRE_MS;
    ...
    this.client.on('connect', () => {
      this.logger.info(`Session redis, connected to redis`);
    });

    this.client.on('error', (err: Error) => {
      if (err.message.indexOf('ECONNREFUSED')) {
        this.logger.warn('Session redis, disconnected');
      } else {
        this.logger.warn(`Session redis, error ${err}`);
      }
    });

    this.client.on('ready', () => {
      this.logger.info('Session redis, ready');
    });
  }

  public async validateToken(request: Request): Promise<ServerToServerSessionInfoDto> {
  ...
  public async createSession(orgId: string, orgCode: string, orgSecret: string, userIp: string, ttl?: number): Promise<string | null> {
  ...
  public async getSession(sessionToken: string): Promise<ServerToServerSessionInfoDto | null> {
    const res = await this.client.get(sessionToken);
    if (res) {
      const ttl = await this.client.ttl(sessionToken);
      const expireIn = ttl && !isNaN(ttl) && Number(ttl) > this.serverTokenExpiration ? Number(ttl) : this.serverTokenExpiration;
      await this.client.expire(sessionToken, expireIn);
      const sessionInfo: ServerToServerSessionInfoDto = JSON.parse(res).sessionInfo;
      return sessionInfo;
    }
    return null;
  }
