...
export class Auth0Service {
  ...
  constructor(private readonly configService: ConfigService) {
    const conn = {
      domain:
        this.configService.get<string>(EnvVariables.AUTH0_MANAGEMENT_DOMAIN) ||
        "",
      clientId:
        this.configService.get<string>(
          EnvVariables.AUTH0_MANAGEMENT_CLIENT_ID
        ) || "",
      clientSecret:
        this.configService.get<string>(
          EnvVariables.AUTH0_MANAGEMENT_CLIENT_SECRET
        ) || "",
    };
  ...
  public async createPasswordChangeTicket(email: string): Promise<void> {
    await axios.post(
      `https://${this.configService.get<string>(EnvVariables.AUTH0_MANAGEMENT_DOMAIN)}/dbconnections/change_password`,
      {
