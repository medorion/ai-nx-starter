...
export class UsersService {
  ...
  getUsers(): Observable<ClientUserDto[]> {
    ...
    return this.apiUsersService.getUsers().pipe(finalize(() => this._loading$.next(false)));
  }

  resetPassword(email: string): Observable<void> {
    this._loading$.next(true);
    const resetPasswordDto: ResetPasswordDto = { email };
    return this.apiUsersService.resetUserPassword(resetPasswordDto).pipe(finalize(() => this._loading$.next(false)));
  }
}
