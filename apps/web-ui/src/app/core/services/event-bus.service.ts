...
export class EventBusService {
  ...
  onDataError(config?: SubscriptionConfig): Observable<EventPayload<{ message: string; code?: string; details?: any }>> {
    return this.pubSubService.subscribe<{ message: string; code?: string; details?: any }>(CommonEvents.DATA_ERROR, config);
  }
   ...
   */
  onUnauthorizedLoginHttp(config?: SubscriptionConfig): Observable<EventPayload<UnauthorizedLoginPayload>> {
    return this.pubSubService.subscribe<UnauthorizedLoginPayload>('http:unauthorized-login', config);
  }
