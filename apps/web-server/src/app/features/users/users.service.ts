...
export class UsersService {
  private static readonly CUSTOMER_ROLES: string[] = [
    Role.Manager,
    Role.Contributor,
    Role.Employee,
  ];
  ...
  protected async getUsersForAvailableOrganizations(
    orgCode: string
  ): Promise<Auth0User[]> {
    const users = await this.auth0Service.getAllUsers();
    return users.filter(
      (user) =>
        user.mdOrganizations.includes(orgCode) || user.mdRole === Role.Root
    );
  }
  ...
  public async getUsersByIds(
    ...
    for (const auth0UserId of auth0UserIds) {
      ...
      if (
        user.mdRole !== Role.Root &&
        !user.mdOrganizations.includes(orgCode)
      ) {
  ...
  public async getUsersByChannel(orgCode: string): Promise<UsersByChannelDto> {
    ...
    const emailUsers = auth0Users
      .filter((user) => user.email)
      .map((user) => this.convertAuth0UserToSeedListItem(user));
    const smsUsers = auth0Users
  ...
  public async getSessions(lastSeconds: number): Promise<UserInSessionDto[]> {
    ...
    const filteredSessions = redisSessions.filter(
      (sessionUsr) => sessionUsr && sessionUsr.sessionInfo
    );
